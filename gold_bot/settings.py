"""Gold bot strategy settings (gold_settings.json)."""
...
SETTINGS_FILE = ROOT / "gold_settings.json"
_lock = threading.RLock()

_DEFAULT_RULES = gold_strategy.GoldRules()
...
def rules_from_settings(settings: dict[str, Any] | None = None) -> gold_strategy.GoldRules:
    s = settings or load_settings()
    base = gold_strategy.GoldRules()
    kwargs: dict[str, Any] = {}
    for field in fields(base):
        if field.name not in s:
            continue
        val = s[field.name]
        if field.name in ("session_start_hour", "session_end_hour") and val == "":
            val = None
